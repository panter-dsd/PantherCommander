...
qWarning("Drive %c: Media has been arrived.", drive.at(0).
...
qWarning("Drive %c: Network share has been mounted.", drive.at(0).

toAscii ()

);
else
qWarning("Drive %c: Device has been added.", drive.at(0).
...
"volumeAdded", Qt::QueuedConnection,
Q_ARG(QString, drive));
}
}
...
qWarning("Drive %c: Media has been removed.", drive.at(0).
...
qWarning("Drive %c: Network share has been unmounted.", drive.at(0).

toAscii ()

);
else
qWarning("Drive %c: Device has been removed.", drive.at(0).
...
QMetaObject::invokeMethod(engine,
"volumeRemoved", Qt::QueuedConnection,
Q_ARG(QString, drive));
}
}
}
...
static void vw_destroy_internal_window (HWND
wnd)
{
if(wnd)
DestroyWindow(wnd);
QString className = QLatin1String ("VolumeWatcher_Internal_Widget") + QString::number (quintptr (vw_internal_proc));
UnregisterClass((wchar_t*)className.
...
WindowsVolumeWatcher::WindowsVolumeWatcher (QObject *parent)
...
bool
WindowsVolumeWatcher::getDiskFreeSpace (const QString &volume, qint64 *total, qint64 *free, qint64 *available) const
{
