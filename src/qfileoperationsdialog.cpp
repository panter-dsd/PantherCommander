...
void QFileOperationsDialog::slotAddingJob(SJob& job)
{
	...
	switch (job.operation)
	{
		...
		case QFileOperationsThread::RemoveDirOperation: {value=dirsCount+filesCount; break;}
		case QFileOperationsThread::CopyFileOperation: {value=QFile(job.params.at(0)).size(); break;}
		case QFileOperationsThread::RemoveFileOperation: {value=1; break;}
		case QFileOperationsThread::MoveFileOperation: {value=QFile(job.params.at(0)).size(); break;}
		case QFileOperationsThread::MoveDirOperation: {value=dirsCount+filesCount+size; break;}
		default: value=1;
	}
//
...
void QFileOperationsDialog::updateListJobs()
{
	...
	for (int i=0; i<qlJobs.count(); i++)
	{
		...
		{
			case QFileOperationsThread::CopyFileOperation: jobText=tr("Copy: "); break;
			case QFileOperationsThread::CopyDirOperation: jobText=tr("Copy: "); break;
			case QFileOperationsThread::RemoveFileOperation: jobText=tr("Remove: "); break;
			case QFileOperationsThread::RemoveDirOperation: jobText=tr("Remove: "); break;
			default:break;
		}
...
//Caption
	...
	{
		case QFileOperationsThread::CopyFileOperation: caption+=tr("Copy"); break;
		case QFileOperationsThread::RemoveFileOperation: caption+=tr("Remove"); break;
		case QFileOperationsThread::CopyDirOperation: caption+=tr("Copy"); break;
		case QFileOperationsThread::RemoveDirOperation: caption+=tr("Remove"); break;
		case QFileOperationsThread::MoveFileOperation: caption+=tr("Move"); break;
		case QFileOperationsThread::MoveDirOperation: caption+=tr("Move"); break;
		default: break;
	}
	...
	else
	{
		...
		switch(currentJob.operation)
		{
			case QFileOperationsThread::CopyFileOperation:
				qlSpeed->setText(tr("Coping %1/s. To job complete %2").arg(getSizeStr(speed)).arg(QTime().addSecs(secs).toString("hh:mm:ss")));
				break;
			case QFileOperationsThread::RemoveFileOperation:
				qlSpeed->setText(tr("Deleting. To job complete %1").arg(QTime().addSecs(secs).toString("hh:mm:ss")));
				break;
			case QFileOperationsThread::CopyDirOperation:
				qlSpeed->setText(tr("Coping %1/s. To job complete %2").arg(getSizeStr(speed)).arg(QTime().addSecs(secs).toString("hh:mm:ss")));
				break;
			...
			case QFileOperationsThread::MoveFileOperation:
				qlSpeed->setText(tr("Moving %1/s. To job complete %2").arg(getSizeStr(speed)).arg(QTime().addSecs(secs).toString("hh:mm:ss")));
				break;
...
{
	if (size<1024) return QString(tr("%1 b ")).arg(size);
	if ((size>=1024) && (size<1048576)) return QString(tr("%1 Kb")).arg(double(size)/1024,0,'f',2);
	if ((size>=1048576) && (size<1073741824))
		return QString(tr("%1 Mb")).arg(double(size)/1048576,0,'f',2);
	if (size>=1073741824) return QString(tr("%1 Gb")).arg(double(size)/1073741824,0,'f',2);
	return "";
}
//
...
#endif
	qApp->processEvents();
	this->resize(this->size().width(),1);
}
//
void QFileOperationsDialog::loadSettings()
{
	QSettings* settings=new QSettings(this);
	this->resize(settings->value("FileOperationsDialog/size",QSize(200,100)).toSize());
	actionShowHideJobList->setChecked(settings->value("FileOperationsDialog/ShowJobsList",false).toBool());
	delete settings;
}
//
...
{
	QSettings* settings=new QSettings(this);
	settings->setValue("FileOperationsDialog/size",this->size());
	settings->setValue("FileOperationsDialog/ShowJobsList",actionShowHideJobList->isChecked());
	...
	switch(qfotOperatinThread->getLastError())
	{
		...
		case QFileOperationsThread::FO_PERMISIONS_ERROR:
			qfocdDialog->setButtons(QFileOperationsConfirmationDialog::Delete |
									...
									QFileOperationsConfirmationDialog::Skip |
									QFileOperationsConfirmationDialog::Cancel);
			qfocdDialog->setText(tr("File %1 is readonly or hidden or system. Delete anyway?").arg(params.at(0)));
			break;
...
QStringList QFileOperationsDialog::normalizeFileNames(const QStringList& names)
{
	...
	{
		list << normalizeFileName(names.at(i));
	}
	return list;
}
//
QString QFileOperationsDialog::normalizeFileName(const QString& name)
{
	...
	while (str.contains(sep))
		str=str.replace(sep,QDir::separator());
	return str;
}
//
