...
void QZipPrivate::fillFileInfo (int index, QZipReader::FileInfo &fileInfo) const
{
    ...
    if (!fileInfo.isDir && !fileInfo.isFile && !fileInfo.isSymLink) {
        ...
        const quint32 efa = qFromLittleEndian<quint32> (&header.h.external_file_attributes[0]);
        if ((mode & 0700) != (0400 | (!(efa & 1) << 7) | ((efa & 0x10) << 2))) {
            fileInfo.permissions |= QFile::Permissions (QFile::ReadOwner | QFile::WriteOwner);
            fileInfo.permissions |= QFile::Permissions (QFile::ReadUser | QFile::WriteUser);
            if ((efa & 0xFF) & 0x10) {
                fileInfo.isDir = 1;
                fileInfo.permissions |= QFile::Permissions (QFile::ExeOwner | QFile::ExeUser);
            } else {
                fileInfo.isFile = 1;
            }
        }
