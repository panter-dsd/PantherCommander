    ...
    if (!_wordSelectionMode && !_lineSelectionMode) {
        ...
        if (right.x () > 0 && !_columnSelectionMode) {
            ...
            if (i >= 0 && i <= _imageSize) {
                ...
                if (selClass == ' ') {
                    while (right.x () < _usedColumns - 1 && charClass (_image[i + 1].character) == selClass &&
...
void TerminalDisplay::mouseDoubleClickEvent (QMouseEvent *ev)
{
    ...
    int selClass = charClass (_image[i].character);
    {
        ...
        x = endSel.x ();
        while (
            ((x < _usedColumns - 1) || (endSel.y () < _usedLines - 1 && (_lineProperties[endSel.y ()] & LINE_WRAPPED)))
            && charClass (_image[i + 1].character) == selClass) {
            i++;
            if (x < _usedColumns - 1) {
                x++;
...
bool TerminalDisplay::focusNextPrevChild (bool next)
{
    if (next) {
        return false;
    } // This disables changing the active part in konqueror
    // when pressing Tab
